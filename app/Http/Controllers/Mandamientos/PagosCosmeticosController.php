...
use Config;
class PagosCosmeticosController extends Controller
{
     private $url=null;
    ...
    public function index(){
      ...
      ,'subtitle'			=> 'Pago Inscripciones Cosméticos'
      ,'breadcrumb' 		=> [
        ['nom'	=>	'Mandamientos', 'url' => '#'],
        ['nom'	=>	'Pago Inscripciones Cosméticos', 'url' => '#']
      ]];
      ...
      if(!Session::has('msnErrorServicio')){
          try{
                ...
                $res = $client->request('POST', $this->url.'mandamiento/consulta/pago',[
                      'headers' => [
                        ...
                        $this->emailErrorOdin('¡Problemas al consultar información de mandamiento de pago Cosméticos! - FECHA '.Carbon::now());
                        return redirect()->route('ver.pagos.cosmeticos');
          }
       ...
       $data['p2']=$p2;
       $data['p3']=$p3;
       return view('recetas.mandamiento.pagoscosmeticos.index',$data);
    ...
    public function store(Request $rq)
    {
        ...
        if($total>=9999){
              return redirect()->route('ver.pagos.cosmeticos')->withErrors("¡Estimado usuario, el sistema genera mandamientos de pagos por un monto inferior a $9,999.99!");
        }
