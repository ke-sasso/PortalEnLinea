...
use Config;
class PagosMedicosController extends Controller
{
     private $url=null;
    ...
    public function index()
    {
      ...
      ,'subtitle'			=> 'Pago Inscripciones Insumos Médicos'
      ,'breadcrumb' 		=> [
        ['nom'	=>	'Mandamientos', 'url' => '#'],
        ['nom'	=>	'Pago Inscripciones Insumos Médicos', 'url' => '#']
      ]];
      ...
      if(!Session::has('msnErrorServicio')){
                ...
                $res = $client->request('POST', $this->url.'mandamiento/consulta/pago',[
                      'headers' => [
                          ...
                          'form_params' =>[
                              ...
                              $this->emailErrorOdin('¡Problemas al consultar información de mandamiento de pago Insumos médicos! - FECHA '.Carbon::now());
                              return redirect()->route('ver.pagos.medico');
            }
       ...
       $data['p2']=$p2;
       $data['p3']=$p3;
       return view('recetas.mandamiento.pagosmedicos.index',$data);
    ...
    public function store(Request $rq){
      //idPago 1.Nacional 2.Extranjero
        ...
        if($total>=9999){
              return redirect()->route('ver.pagos.medico')->withErrors("¡Estimado usuario, el sistema genera mandamientos de pagos por un monto inferior a $9,999.99!");
        }
      ...
      try {
          ...
          $mandamiento=Mandamiento::find($last);
            ...
            $pdf->SetTitle('MANDAMIENTO - '.$mandamiento->ID_MANDAMIENTO);
    				...
    				$pdf->SetFont('Times','',7);
    				$pdf->Cell(0,4,'UNIDAD DE INSUMOS MEDICOS  - PAGOS VARIOS WEB',0,1,'C');
    				$pdf->SetFont('Times','',7);
    				$x = $pdf->GetX();
    				$y = $pdf->GetY();
