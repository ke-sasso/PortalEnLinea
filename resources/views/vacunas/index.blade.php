      ...
      <div class="modal-body">
          <div align="center">
            ...
            @if(Session::get('SinCerificar')==1)
              <label>Se ha ingresado su solicitud, debera visitar la DNM para continuar con el tramite.</label> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
            @elseif(Session::get('desestimiento')!=0)
              ...
              <a href="{{route('desistimiento.sim',['idSolicitud' => Session::get('desestimiento')])}}" target="_blank" title="Imprimir Solicitud"><i class="fa fa-print icon-rounded icon-xs icon-primary"></i></a>
    ...
    <div role="tabpanel">
      ...
      <ul class="nav nav-tabs" role="tablist">
        <li role="presentation" class="active">
          <a href="#panel-enlinea" aria-controls="panel-enlinea" role="tab" id="tabTramite" data-toggle="tab">VENTANILLA VIRTUAL</a>
        </li>
        <li role="presentation">
          <a href="#panel-express" aria-controls="panel-express" role="tab" id="tabTramite" data-toggle="tab">VENTANILLA EXPRESS</a>
        </li>
        <li role="presentation">
                ...
                @foreach($tramites as $tra)
                  ...
                  <tr>
                    <td id="idTipoTramite">
                      ...
                      <a class="btn btn-primary btn-sm" onclick="getTabGeneral({{$tra->id}},{{$tra->idClasificacion}});">
        ...
        var dtproductos = $('#dt-codmods').DataTable({
                          ...
                          columns:[                        
                                  ...
                                  {data: 'descripcion',name:'descripcion'},
                                  {
                                    "mData": null,
                                    "bSortable": false,
                                    "mRender": function (data,type,full) { return '<input type=checkbox value="test your look ;)" name="idproducto" data-cod="'+data.codigos+'"  data-mod="'+data.modelos+'" data-descripcion="'+data.descripcion+'" data-id_producto_codmod="'+data.id_producto_codmod+'" class="ckb-check">' }
                                  },
   ...
   //console.log(id_tramite);
    if(id_tramite==28){
      ...
      if(!$('#txtidproducto').val()) {
        ...
        //console.log('adiccion de acondicionador');
        if(!$('#descripcion4').val()){
            alertify.alert("Mensaje de sistema","Debe digitar el nuevo fabricante alterno para realizar el tramite!");
        }
        else{
          guardar.call();
        }
        ...
        if (!$('input.chkPrese').is(':checked')) {
          if(id_tramite==17){
            ...
            alertify.alert("Mensaje de sistema","Debe seleccionar un fabricante alterno a eliminar para realizar el tramite");  
          }
        }
        else{
          guardar.call();
        ...
        /*else if(!$('#descripcion').val()){
            alertify.alert("Mensaje de sistema","Debe digitar el la nueva dirección del fabricante seleccionado para realizar el tramite!");
        }*/
        else{
          guardar.call();
        }
                    ...
                    if(r.data.length!=0){
                      for(j=0;j<r.data.length;j++){
                            ...
                            if(id_tramite==18 || id_tramite==17){
                                $('#validar').show();
                                $('#guardarSoli').show();
                                   $('#fabs').append('<tr><td><input type="checkbox" class="chkPrese" name="idFab[]" value="'+r.data[j].ID_FABRICANTE+'"></td><td>' + r.data[j].NOMBRE_FABRICANTE +'</td><td>' + r.data[j].TIPO_FABRICANTE +'</td><td>' + r.data[j].NOMBRE_PAIS +'</td></tr>');
                            }
                            if(id_tramite==9){
                        ...
                        if(id_tramite==11 || id_tramite==18){
                            alertify.alert("Mensaje de sistema - Error", "No puede realizar el tramite con el producto seleccionado no poseé fabricante alterno!");
                                $('#guardarSoli').hide();
                                $('#validar').hide();
                        }
                          ...
                          $('#fabricantes').append('<tr class="table-warning"><td class="table-warning">ESTE PRODUCTO NO POSEÉ FABRICANTES ALTERNOS</td></tr>');
                        }
...
$("#documentos tr").remove(); 
   ...
   if(idTramite==13){
     ...
     // $('#descripcion').val('');
      if(idTramite==6){ 
        ...
        $("#tit611").text('AGREGRE EL NUEVO FABRICANTE ALTERNO:'); 
      }
      else if(idTramite==11){
        $("#title611").text('SELECCIONE EL FABRICANTE ALTERNO A CAMBIAR:'); 
      ...
      $('#tram18').show();
      if(idTramite==18){
        $("#title1817").text('SELECCIONE EL FABRICANTE ALTERNO A ELIMINAR:'); 
      }
  ...
  function guardarCodigos(){
    ...
    for (var i =0; i< dataAddProds.length;i++) {
      ...
      $('#vwCodMods').append('<tr><td><input type="hidden" name="codmod[]" value="'+dataAddProds[i][3]+'">'+dataAddProds[i][0]+'</td><td>'+dataAddProds[i][1]+'</td><td>'+dataAddProds[i][2]+'</td></tr>');
