...
<div class="panel panel-success"> 
  ...
  <div class="panel-body">  
    ...
    <div class="table-responsive">    
      <table style="font-size: 12px;" id="dt-tramites-post-sim" class="table table-hover table-striped" role="group" width="100%">
        <thead>
          <tr>
            <th># SOLICITUD</th>
            <th># REGISTRO</th>
...
$( document ).ready(function() {
    ...
    var table = $('#dt-tramites-post-sim').DataTable({
       "pageLength": 5,
        processing: true,
        serverSide: true,
        "searching": false,
        ajax: {
            url: "{{ route('dt.data.solicitudes.sim.post') }}",
            data: function (d) {
                d.nsolicitud= $('#nsolicitud').val();
                d.nregistro= $('#nregistro').val();
                d.nomComercial= $('#nomComercial').val();
                d.tramite= $('#tramite').val();
                d.estado= $('#estado').val();
            }
        },
        columns: [
            ...
            {data: 'TRAMITE', name: 'TRAMITE',orderable:false},
