...
<div class="panel panel-success">
	...
	<div class="panel-body">
		<div id="smartwizard">
		    ...
		    <div>
		        <div id="step-1" class="">
                    @include('registro.nuevoregistro.pasosEdit.paso1.mandamiento')
		        </div>
		        <div id="step-2" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso2.gnralProducto')
		        </div>
		        <div id="step-3" class="">
                    @include('registro.nuevoregistro.pasosEdit.paso3.mainpaso3')
		        </div>
		        <div id="step-4" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso4.fabricantes')
		        </div>
		        <div id="step-5" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso5.certificadolibreventa')
		        </div>
		        <div id="step-6" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso6.certificadobpm')
		        </div>
		        <div id="step-7" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso7.distribuidor')
		        </div>
		        <div id="step-8" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso8.materialempaque')
		        </div>
		        <div id="step-9" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso9.farmacologica')
		        </div>
		        <div id="step-10" class="">
		            @include('registro.nuevoregistro.pasosEdit.paso10.documentos')
		        </div>
		        <div id="step-11" class="">
                @include('registro.nuevoregistro.pasosEdit.paso11.finalizacion')
		        </div>
		    </div>
		</div>
...
@include('registro.nuevoregistro.pasosEdit.mainJs')
@include('registro.nuevoregistro.pasosEdit.guardarstepbyJs')
...
<script type="text/javascript">
  ...
  var globalBpmFabPrinActivos=[];
        ...
        routes: [
            {
              ...
              getExpedientesDocumentos: "{{route('get.editar.expedientes.documentos',['idSolicitud'=>Crypt::encrypt($solicitud->idSolicitud),'vista'=>'1'])}}",
	...
	$(document).ready(function(){
        ...
        if (performance.navigation.type == 1) {
                ...
                window.location = '{{route("get.preregistrorv.index.editarSolicitud",['idSolicitud'=>Crypt::encrypt($solicitud->idSolicitud)])}}';
         }
  ...
  function cambiar(row){
    ...
    if(id==5 || id==7 || id==18){
      docobli='Excepto para productos de origen Reconocimiento Mutuo Centroamericano';
    }
                            ...
                            if(obligatorio==1){
                              ...
                              /*$('#file' + id).fileinput({
                                ...
                                progressClass: "hide",
                                        ...
                                        $('#kv-success-' + $(this).attr('id')).html('<button name="eliminarDocAjax" id="eliminarDocAjax" title="Eliminar Documento" onclick="eliminarDocumento(this);" data-id="'+id+'" class="btn btn-danger eliminarDocAjax"><i class="fa fa-trash" aria-hidden="true"></i>ELIMINAR DOCUMENTO</button> <span id="spanfs-file'+id+'" class="label label-succes">Se subió el archivo correctamente</span>').show();
                                        $('#kv-error-file' + id).empty();
                                        $('#kv-error-file' + id).hide();

                                }).on('fileuploaderror', function(event, data, msg) {
                                    //$('.'+pgbarid).remove();
                                    $('#kv-error-' + $(this).attr('id')).html('<span id="spanf-file'+id+'" class="label label-danger">Problemas en subir archivo</span>').show();
...
@include('registro.nuevoregistro.pasosEdit.cargasJs')
@endsection
