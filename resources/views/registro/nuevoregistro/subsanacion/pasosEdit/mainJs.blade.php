...
$('#btnAddPresentacion').click(function(event) {
    $("#empa1").val('');$("#por1").val('');$("#mat1").val('');$("#empa2").val('');$("#por2").val('');
    $("#mat2").val('');$("#empa3").val('');$("#por3").val('');$("#mat3").val('');$("#material").val('');
    $("#color").val('');$("#accesorios").val('');$("#tipoP").val(1);$("#presentacionespecial").val('');
    $('#empesecfalse').click();$('#empeterfalse').click();
...
$('#btnAddPresentacionDt').click(function(event) {
    ...
    if(values.checkempsec==0 && values.checkempter==0){
        if(values.empa1 != 'null'  && values.por1.length > 0  && values.mat1 != 'null' && values.tipoP.length > 0  && values.material != 'null'  && values.color != 'null'){
            ...
            $('#dlgAddPresent').modal('toggle');
            $('#toke-presentaciones').append('<input form="RegistroStep1y2" type="hidden" class="form-control" id="presentaciones2" name="presentaciones2" value="{{Crypt::encrypt('presentaciones2') }}"/>');
        }
        else{
...
$('#btnAddPrincipioA').click(function(event) {
     ...
     if(matprimaid  && concentracion && unimedidaid){
         ...
         $('#dlgAddPinicipio').modal('toggle');
         $('#toke-principios').append(' <input  form="RegistroStep1y2" type="hidden" class="form-control" id="principios2" name="principios2" value="{{Crypt::encrypt('principios2') }}" />');
     }else{
...
$('input[type=radio][name=tipoTitular]').change(function(){
    ...
    var tokentit = document.getElementById('habilitar-busRepresetante');
    var tokenapo = document.getElementById('habilitar-poderApoderado');
    if(tipotitular==1){
        tokentit.style.display = 'none';
        tokenapo.style.display = 'none';
        $("#poderProfesional2").remove();
        $("#poderApoderado2").remove();
        @if(in_array('poderRepresentante', $campos))  tokentit.style.display = 'block';  @endif
        @if(in_array('poderApoderado', $campos)) tokenapo.style.display = 'block'; @endif

    }else{
                @if(!empty($campos))
                     @if(in_array('idTitular', $campos))
                    tokentit.style.display = 'block';
                    tokenapo.style.display = 'block';
                    @elseif(in_array('poderRepresentante', $campos))
                        tokentit.style.display = 'block';
                    @elseif(in_array('poderApoderado', $campos))
                      tokenapo.style.display = 'block';
                    @endif
...
$('#searchbox-titular').on('change', function() {
    $('#token-idTitular').append('<input  form="RegistroPreStep3" type="hidden" class="form-control" id="idTitular2" name="idTitular2" value="{{Crypt::encrypt('idTitular2') }}" />');
    $('#bodyTitular').empty();
...
$('#validarPoderP').click(function(event){
    ...
    $.ajax({
        ...
        success:  function (r){
            ...
            $('#token-poderProfesional').append('<input  form="RegistroPreStep3" type="hidden" class="form-control" id="poderProfesional2" name="poderProfesional2" value="{{Crypt::encrypt('poderProfesional2') }}" />');
...
$('#validarPresentanteL').click(function(event) {
    ...
    $.ajax({
        ...
        success:  function (r){
            ...
            $('#token-poderRepresentante').append('<input  form="RegistroPreStep3" type="hidden" class="form-control" id="poderRepresentante2" name="poderRepresentante2" value="{{Crypt::encrypt('poderRepresentante2') }}" />');
...
$('#validarApoderado').click(function(event) {
    ...
    $.ajax({
        ...
        success:  function (r){
            ...
            $('#token-poderApoderado').append('<input  form="RegistroPreStep3" type="hidden" class="form-control" id="poderApoderado2" name="poderApoderado2" value="{{Crypt::encrypt('poderApoderado2') }}" />');
...
function ajaxViews(ruta){
    $.ajax({
        type: 'GET',
        url:  ruta,
        data:{idSolicitud:"{{Crypt::encrypt($solicitud->idSolicitud)}}",idvista:2},
        success:  function (r){
...
$('#searchbox-fabricante2').on('change', function() {
    $('#token-nombreFabAlterno').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreFabAlterno2" name="nombreFabAlterno2" value="{{Crypt::encrypt('nombreFabAlterno2') }}" />');
...
$('#searchbox-fabricante3').on('change', function() {
     $('#token-nombreLabAcondicionador').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreLabAcondicionador2" name="nombreLabAcondicionador2" value="{{Crypt::encrypt('nombreLabAcondicionador2') }}" />');
...
$("#dt-fabricantesAcondicionador").on('click', '.btnEliminar', function (){
    $("#nombreLabAcondicionador2").remove();
    $(this).closest('tr').remove();
    $('#token-nombreLabAcondicionador').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreLabAcondicionador2" name="nombreLabAcondicionador2" value="{{Crypt::encrypt('nombreLabAcondicionador2') }}" />');
});
...
$('#searchbox-fabricante1').on('change', function(){
    $('#token-labprincipal').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreFabricante2" name="nombreFabricante2" value="{{Crypt::encrypt('nombreFabricante2') }}" />');
...
$('#searchbox-distribuidor').on('change', function() {
     $("#distribuidores2").remove();
     $('#token-distribuidores').append('<input  form="RegistroPreStep7" type="hidden" class="form-control" id="distribuidores2" name="distribuidores2" value="{{Crypt::encrypt('distribuidores2') }}" />');
...
$('#searchbox-fabricante4').on('change', function () {
    $('#token-nombreLabRelacionado').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreLabRelacionado2" name="nombreLabRelacionado2" value="{{Crypt::encrypt('nombreLabRelacionado2') }}" />');
...
function armarPresentacion(){
    ...
    if(emp2==0 && emp3==0){
        if(empaque1!='' && material1!='')
            texto= empaque1  +  ' ' + material + ' ' + color + ' X ' + por1 + ' '  + material1 + ' '+ accesorios;
    }
    else if(emp2==1 && emp3==0){
        texto= empaque2 + ' X '+ por2 + ' ' + empaque1  +  ' ' + material + ' ' + color + ' X ' + por1 + ' '  + material1 + ' '+ accesorios;
    }
    else if (emp2==1 && emp3==1){
        texto= empaque3 + ' X '+ por3 + ' ' + empaque2 + ' X ' + por2 + ' ' + empaque1  +  ' ' + material + ' ' + color + ' X ' + por1 + ' '  + material1 + ' '+ accesorios;
    }
...
$('#searchbox-fabricante5').on('change', function () {
    ...
    if(principio===null){
        ...
        $('#token-nombreFabPrincipio').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="nombreFabPrincipio2" name="nombreFabPrincipio2" value="{{Crypt::encrypt('nombreFabPrincipio2') }}" />');
...
$('#searchbox-poderMaquila1').on('change', function() {
    ...
    $('#token-poderFabprincipal').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="poderFabPrincipal2" name="poderFabPrincipal2" value="{{Crypt::encrypt('poderFabPrincipal2') }}" />');
});
...
$("#dt-poderFabAlterno").on('click', '.btnEliminar', function () {
    $(this).closest('tr').remove();
     $('#token-poderFabAlterno').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="poderFabAlterno2" name="poderFabAlterno2" value="{{Crypt::encrypt('poderFabAlterno2') }}" />');
});
...
$("#dt-poderFabAcondicionador").on('click', '.btnEliminar', function () {
    $(this).closest('tr').remove();
     $('#token-poderFabAcondicionador').append('<input  form="RegistroPreStep4" type="hidden" class="form-control" id="poderFabAcondicionador2" name="poderFabAcondicionador2" value="{{Crypt::encrypt('poderFabAcondicionador2') }}" />');
});

function dataStep2(){
    //METODO PARA LISTAR LOS TIPOS DE MEDICAMENTOS
    SelectTipoMedicamento({{$solicitud->solicitudesDetalle->innovador}});
...
function dataStep8(){
        ...
        $('#dt-fabricantesAcondicionador tbody tr').each(function(){
            ...
            @if ($solicitud->materialEmpaque!=null)
              idSelected = '{{$solicitud->materialEmpaque->idAcondicionador}}';
            @endif
             /* Obtener todas las celdas */
              var celdas = $(this).find('td');
              var valueinput = $(this).find('input').val();
              if (idSelected==valueinput) {
                selected='selected';
              }
           $('#listLabAcondicionador').append('<option '+selected+' value="'+valueinput+'">'+$(celdas[0]).html()+' - ('+$(celdas[2]).html()+')'+'</option>');
...
function dataStep6(){
    ...
    $('#idcertificadobpm').val(id2);
     ...
     $('#dt-fabricantesAlternos tbody tr').each(function(){
            ...
            var valueinput = $(this).find('input').val();
            var valueEmision = '';
            var valueVencimiento = '';
            var nomemi = $(celdas[2]).html();
            if (globalBpmAlterno.length>0) {
              for (var i = 0; i < globalBpmAlterno.length; i++) {
                if (globalBpmAlterno[i].idAlterno==valueinput) {
                  valueEmision =globalBpmAlterno[i].fechaEmision;
                  valueEmision =valueEmision.substring(0, 10);
                  valueVencimiento = globalBpmAlterno[i].fechaVencimiento;
                  valueVencimiento =valueVencimiento.substring(0, 10);
                  nomemi = globalBpmAlterno[i].nombreEmisor;
                  if(valueEmision === "1900-01-01"){ valueEmision=''; }
                  if(valueVencimiento ==="1900-01-01"){valueVencimiento=''; }
                }
              }
            }
         $('#dt-pract-labfabalterno tbody').append('<tr><input form="RegistroPreStep6" type="hidden" name="practLabAlternos[]" value="'+valueinput+'"><td>'+$(celdas[0]).html()+'</td><td><input class="form-control" name="emisorAlterno-'+valueinput+'" value="'+nomemi+'" type="hidden" form="RegistroPreStep6" />'+nomemi+'</td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaEmision-'+valueinput+'"  name="fechaEmision-'+valueinput+'" placeholder="dd-mm-yy" value="'+valueEmision+'"></td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaVencimiento-'+valueinput+'"  name="fechaVencimiento-'+valueinput+'" placeholder="dd-mm-yyyy" value="'+valueVencimiento+'"></td><tr>');
      ...
      $('#dt-fabricantesAcondicionador tbody tr').each(function(){
            ...
            var valueEmision = '';
            var valueVencimiento = '';
            var noemi=$(celdas[2]).html();
            if (globalBpmAcondicionador.length>0) {
              for (var i = 0; i < globalBpmAcondicionador.length; i++) {
                if (globalBpmAcondicionador[i].idAcondicionador==valueinput) {
                  valueEmision =globalBpmAcondicionador[i].fechaEmision;
                  valueEmision =valueEmision.substring(0, 10);
                  valueVencimiento = globalBpmAcondicionador[i].fechaVencimiento;
                  valueVencimiento =valueVencimiento.substring(0, 10);
                  noemi =  globalBpmAcondicionador[i].nombreEmisor;
                  if(valueEmision==="1900-01-01"){ valueEmision=''; }
                  if(valueVencimiento==="1900-01-01"){ valueVencimiento=''; }
                }
              }
            }
         $('#dt-pract-labacondicionador tbody').append('<tr><input form="RegistroPreStep6" type="hidden" name="practLabAcondi[]" value="'+valueinput+'"><td>'+$(celdas[0]).html()+'</td><td><input class="form-control" name="emisorAcondicionador-'+valueinput+'" value="'+noemi+'" type="hidden" form="RegistroPreStep6" />'+noemi+'</td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaEmision-'+valueinput+'"  name="fechaEmision-'+valueinput+'" placeholder="dd-mm-yy" value="'+valueEmision+'"></td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaVencimiento-'+valueinput+'"  name="fechaVencimiento-'+valueinput+'" placeholder="dd-mm-yyyy" value="'+valueVencimiento+'"></td><tr>');
          ...
          $('#dt-fabricantesRelacionados tbody tr').each(function(){
            ...
            var valueVencimiento = '';
            var noemi=$(celdas[2]).html();
            if (globalBpmRelacionados.length>0) {
              for (var i = 0; i < globalBpmRelacionados.length; i++) {
                if (globalBpmRelacionados[i].idRelacionado==valueinput) {
                  valueEmision =globalBpmRelacionados[i].fechaEmision;
                  valueEmision =valueEmision.substring(0, 10);
                  valueVencimiento = globalBpmRelacionados[i].fechaVencimiento;
                  valueVencimiento =valueVencimiento.substring(0, 10);
                  noemi =  globalBpmRelacionados[i].nombreEmisor;
                  if(valueEmision==="1900-01-01"){ valueEmision=''; }
                  if(valueVencimiento==="1900-01-01"){ valueVencimiento=''; }
                }
              }
            }
         $('#dt-pract-labRelacionado tbody').append('<tr><input form="RegistroPreStep6" type="hidden" name="practLabRelacionado[]" value="'+valueinput+'"><td>'+$(celdas[0]).html()+'</td><td><input class="form-control" name="emisorRelacionado-'+valueinput+'" value="'+noemi+'" type="hidden" form="RegistroPreStep6" />'+noemi+'</td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaEmisionRel-'+valueinput+'"  name="fechaEmisionRel-'+valueinput+'" placeholder="dd-mm-yy" value="'+valueEmision+'"></td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaVencimientoRel-'+valueinput+'"  name="fechaVencimientoRel-'+valueinput+'" placeholder="dd-mm-yyyy" value="'+valueVencimiento+'"></td><tr>');
      ...
      $('#dt-fabricantesPrincipioActivo tbody tr').each(function(){
            ...
            var valueVencimiento = '';
            var noemi=$(celdas[2]).html();
            if (globalBpmFabPrinActivos.length>0) {
              for (var i = 0; i < globalBpmFabPrinActivos.length; i++) {
                if (globalBpmFabPrinActivos[i].idFabActivo==valueinput) {
                  valueEmision =globalBpmFabPrinActivos[i].fechaEmision;
                  valueEmision =valueEmision.substring(0, 10);
                  valueVencimiento = globalBpmFabPrinActivos[i].fechaVencimiento;
                  valueVencimiento =valueVencimiento.substring(0, 10);
                  noemi =  globalBpmFabPrinActivos[i].nombreEmisor;
                  if(valueEmision==="1900-01-01"){ valueEmision=''; }
                  if(valueVencimiento==="1900-01-01"){ valueVencimiento=''; }
                }
              }
            }
         $('#dt-pract-FabPrinActivo tbody').append('<tr><input form="RegistroPreStep6" type="hidden" name="practLabPrinActivo[]" value="'+valueinput+'"><td>'+$(celdas[0]).html()+'</td><td><input class="form-control" name="emisorPrinActivo-'+valueinput+'" value="'+noemi+'" type="hidden" form="RegistroPreStep6" />'+noemi+'</td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaEmisionAct-'+valueinput+'"  name="fechaEmisionAct-'+valueinput+'" placeholder="dd-mm-yy" value="'+valueEmision+'"></td><td><input form="RegistroPreStep6" type="date" class="form-control datepicker" id="fechaVencimientoAct-'+valueinput+'"  name="fechaVencimientoAct-'+valueinput+'" placeholder="dd-mm-yyyy" value="'+valueVencimiento+'"></td><tr>');
...
function seleccionoArchivo(id){
    $('#docActua'+id).remove();
    $('#token-doc').append('<input type="hidden" id="docActua'+id+'"  id="docActua'+id+'" form="RegistroPreStep10" value="docActua'+id+'" />');
}
function validarDocumentosObservados(){
        var files = true;
        $.each($('input[type=file]'),function(){
              var na = $(this).attr('name');
              //NO VALIDAMOS LOS DOCUMENTOS QUE SON DE NOTA ACLARATORIA
              if(na!='file-es[49]' && na!='file-es[50]' && na!='file-es[51]'){
