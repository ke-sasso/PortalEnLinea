  ...
  <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
      <div class="table-responsive">
        <table class="table table-hover table-sm" id="principiosactivos">
            <caption>
               @if(!empty($campos)) @if(in_array('principios', $campos))
                  <a class="btn btn-danger btn-perspective btn-xs vals" id="principios-val"><i class="fa  fa-times"></i></a>
                  <div id="toke-principios"></div>
                 @endif @endif<b>AGREGAR PRINCIPIOS ACTIVOS</b></caption>
                   <caption>
          ...
          <thead>
            <tr>
              ...
              <th>CONCENTRACIÓN</th>
              <th>U/M</th>
              @if(!empty($campos)) @if(in_array('principios', $campos)) <th>OPCIONES</th> @endif  @endif
            </tr>
          </thead>
          <tbody>
            @if ($solicitud->principiosactivos!=null)
              @foreach ($solicitud->principiosactivos as $principio)
                <tr>
                  ...
                  @if(!empty($campos)) @if(in_array('principios', $campos)) <td><button class="btn btn-danger btnEliminar"><i class="fa fa-trash" aria-hidden="true"></i></button></td> @endif  @endif
                </tr>
              @endforeach
            @endif
          </tbody>
